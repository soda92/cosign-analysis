...
def decode_flow(flow_data):
    ...
    method = (
        req.get("method", b"").decode()
        if isinstance(req.get("method"), bytes)
        else str(req.get("method"))
    )
    scheme = (
        req.get("scheme", b"").decode()
        if isinstance(req.get("scheme"), bytes)
        else str(req.get("scheme"))
    )
    host = (
        req.get("host", b"").decode()
        if isinstance(req.get("host"), bytes)
        else str(req.get("host"))
    )
    port = req.get("port")
    path = (
        req.get("path", b"").decode()
        if isinstance(req.get("path"), bytes)
        else str(req.get("path"))
    )
